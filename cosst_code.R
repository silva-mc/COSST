# R code to run the Climate-Oriented Seed Sourcing Tool (COSST)
...
# STEP 1: USER-DEFINED ARGUMENTS

# Select the species
# We recommend the user to check the name of the species on GBIF: https://www.gbif.org/
species = "Caryocar brasiliense Cambess." # Pequi

# Specify the coordinates of the restoration site (latitude, longitude)
site = data.frame(lat = -14.350088458305477, long = -48.400013676905374) # lat, long - Central Brazil

# Select a provenancing strategy: "compo" for composite, "clima.adjus" for climate-adjusted, "predi" for predictive
provenance = "clima.adjus"

# Select a Shared Socioeconomic Pathway (SSP): "126" for an optimistic scenario (SSP1), "585" for a pessimistic scenario (SSP5)
ssp = "585"

# Select a time frame: "2021-2040", "2041-2060", "2061-2080", "2081-2100"
time = "2021-2040"

# OPTIONAL: Read the seed sourcing sites coordinates
setwd("...")
seed.supp = read.csv("seed.supp.csv")
...

{
  ...
  # The default extent is the country containing the site, but the user can replace it with another SpatialPolygonsDataFrame object
...
{
  # Another algorithm can be used if it provides weights for the environmental variables
  ...
  # # Model performance
  # cbi = evalContBoyce(pres = stats::na.omit(raster::extract(x = projection, # Calculate CBI
  #                                                           y = occur.valid)),
  #                     contrast = stats::na.omit(raster::extract(x = projection,
  #                                                               y = background)))
...

{
  ...
  clima.match = raster::mask(x = raster::crop(x = clima.match, 
                                              y = proje.restr),
                             mask = proje.restr) # Change to another mask if binary SDM should be avoided
  ...
  # # Display the climatic uncertainty score
  # print(clima.uncer)
  ...
  # # Create an continuum between the predictive (clima.match) and composite (geogr.proxi) layers
  # 
  # # Define a weight to the predictive layer, 1 being the maximum importance and 0 the minimum importance (clima.weigh = 0 returns to the composite strategy)
  # clima.weigh = .5
  # 
  # # Flexible climate-adjusted layer
  # clima.adjus.flexi = ((clima.match * clima.uncer) * clima.weigh) + geogr.proxi
  # 
  # # Normalize it
  # clima.adjus.flexi = (clima.adjus.flexi - clima.adjus.flexi@data@min) / (clima.adjus.flexi@data@max - clima.adjus.flexi@data@min)
...

{
  ...
  cosst.supp.summ = cosst.supp %>%
    group_by(supplier) %>%
    summarise(cosst.perce = sum(cosst.perce, na.rm = T))
